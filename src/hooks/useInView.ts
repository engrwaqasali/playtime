/**
 * Copy-paste from https://github.com/bitmap/react-hook-inview
 */

import { useEffect, useState, useRef, useCallback, RefObject, Dispatch, SetStateAction, ComponentState } from 'react';
...
type Hook = [SetRef, UseInViewState['inView'], UseInViewState['entry'], IntersectionObserver | null];
...
export const useInViewEffect: UseInViewEffect = (callback, options = {}, externalState = []) => {
...
export const useInView: UseInView = (options, externalState = []) => {
    ...
    const callback: IntersectionObserverCallback = ([entry], observer): void => {
        ...
        if (intersectionRatio >= 0) {
            const inThreshold = observer.thresholds.some(t => intersectionRatio >= t);
            ...
            if (inView) {
                if (onEnter) onEnter(entry, observer);
